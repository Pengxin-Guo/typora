...
38. 常用`STL`
    ...
    - `vector`：`push_back`，在末尾加入一个元素；`pop_back`，在末尾弹出一个元素；`size`，获取长度；`clear`，清空；

      注：`clear`函数只是对这片存储区进行了清空操作，并不会回收内存，需要回收内存的话，用`swap`，如：
    ...
    - `set`：`insert`，插入一个元素；`erase`，删除一个元素；`count`，统计集合中某个元素的个数；`size`，获取元素个数；`clear`，清空；迭代器用法如下：
    ...
    - `map`：`insert`，插入一对映射`pair`，`pair`通过`make_pair`获得；`count`，判断关键字是否存在；`size`，获取映射对个数；`clear`，清空；

      注：`map`可通过下标直接获取元素，如果这个映射不存在，系统会自动创建一个这个映射；`map`通过迭代器输出每个映射对时，`first`代表映射对的键，`second`代表映射对的值；
